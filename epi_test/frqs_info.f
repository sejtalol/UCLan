...
c      read(nnsn)(mass(iq),iq=1,n_sp)
c      read(nnsn)(mang(iq),iq=1,n_sp)
c      read(nnsn)(mpe(iq),iq=1,n_sp)
c      read(nnsn)(mte(iq),iq=1,n_sp)
...
c      enddo
       ...
       nnch=36
       write(filename,"('frqs_info_t',I3.3,'.dat')")iunit
...
c        print *, snp, istep, n_sp
        ...
        do i = 1, tmpn, 1
           ...
           write(nnch,10) istep, myrad(i), myomega(i), mykappa(i),      &
     &  mykappaz(i)
10         format(2X, I6, F15.8, F15.8, F15.8, F15.8)
        end do
c        print *,istep
       end do
