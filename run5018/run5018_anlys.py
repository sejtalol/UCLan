...
ej_x_min, ej_x_max, ej_y_min, ej_y_max, dej_y_min, dej_y_max = -5, 0 , -5, 5, -1.5, 1.5

# L_Z: lz_x = ang, lz_y = da, dlz_y = da/ang
lz_x_min, lz_x_max, lz_y_min, lz_y_max, dlz_y_min, dlz_y_max = -1, 7, -5, 5, -1.5, 1.5

#### dEj - Ej #### hist2D
fig1, ax1 = plt.subplots()
mod.hist2d_plot(Ej, dEj, ej_x_min, ej_x_max, ej_y_min, ej_y_max, ax = ax1)
lab = 'E_{J}'
ax1.set_xlabel('$%s$'%(lab))
ax1.set_ylabel('$\Delta %s$'%(lab))
ax1.set_title('Change of $%s$ in %s from T = %d to %d'%(lab, keyword, nn, nn+dn))
...
ax2.set_title('Change of $%s$ in %s from T = %d to %d'%(lab, keyword, nn, nn+dn))
fig2.savefig('./output/d%s_%s_%s_t%d_t%d_.png'%(lab, lab, keyword, nn, nn+dn))

#### dLZ - LZ #### hist2D
fig3, ax3 = plt.subplots()
mod.hist2d_plot(ang, da, lz_x_min, lz_x_max, lz_y_min, lz_y_max, ax = ax3)
lab = 'L_{Z}'
ax3.set_xlabel('$%s$'%(lab))
ax3.set_ylabel('$\Delta %s$'%(lab))
ax3.set_title('Change of $%s$ in %s from T = %d to %d'%(lab, keyword, nn, nn+dn))
...
ax4.set_title('Change of $%s$ in %s from T = %d to %d'%(lab, keyword, nn, nn+dn))
